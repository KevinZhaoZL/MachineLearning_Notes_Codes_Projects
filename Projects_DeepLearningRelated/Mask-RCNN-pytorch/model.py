# -*- coding: utf-8 -*-
# @Time    : 19-1-16 上午10:37
# @Author  : Zhao Lei
# @File    : model.py
# @Desc    :The main Mask R-CNN model implemenetation.
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    flag = torch.nonzero(gt_class_ids < 0).size()
    if flag[0]:
...
class Classifier(nn.Module):
    ...
    def forward(self, x, rois):
        a = rois.size()[0]
        if a == 0:
            rois = torch.tensor([[0., 0., 0., 0.],
                                 [0., 0., 0., 0.]], device='cuda:0')
        flag = [rois] + x
        # if flag
        x = pyramid_roi_align(flag, self.pool_size, self.image_shape)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        x = self.conv2(x)
